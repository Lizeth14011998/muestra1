...
var i,
	...
	atributos = "\\ [" + espacio en blanco + "* (" + identificador + ") (?:" + espacio en blanco +
		...
		// "Los valores de atributo deben ser identificadores CSS [captura 5]
	...
	matchExpr = {
		...
		"NIÑO": nueva RegExp ("^ :( solo | primero | último | enésimo | enésimo-último) - (niño | de tipo) (?: \\ (" +
...
función Sizzle (selector, contexto, resultados, semilla) {
	var m, i, elem, nid, coincidencia, grupos, newSelector,
		...
		// nodeType tiene el valor predeterminado de 9, ya que el contexto predeterminado es document
...
/ **
 * Apoyar las pruebas usando un elemento
...
Expr = Sizzle.selectors = {
					...
					if (padre) {
						...
						if (reenviar && useCache) {
							...
							while ((nodo = ++ nodeIndex && nodo && nodo [dir] ||
								...
								// Cuando se encuentra, los índices de caché en `parent` y se rompen
	...
	pseudos: {
		...
		"no": markFunction (función (selector) {
			...
			return matcher [expando]?
				markFunction (función (semilla, coincidencias, _context, xml) {
					var elem,
						inigualable = coincidente (semilla, nulo, xml, []),
				...
				función (elem, _context, xml) {
					...
					// No te quedes con el elemento (número 299)
					entrada [0] = nulo;
					return! results.pop ();
				};
		}),
		...
		"comprobado": función (elem) {
			...
			// En CSS3,: check debería devolver tanto los elementos marcados como los seleccionados
			// http://www.w3.org/TR/2011/REC-css3-selectors-20110929/#checked
		...
		"botón": función (elem) {
			var name = elem.nodeName.toLowerCase ();
			return nombre === "entrada" && elem.type === "botón" || nombre === "botón";
		},
	...
	while (soFar) {
		...
		if (! coincidió || (coincidencia = rcomma.exec (soFar))) {
			si (coincidencia) {
				...
				soFar = soFar.slice (coincide con [0] .length) || hasta ahora;
			}
			grupos.push ((tokens = []));
		}
...
compile = Sizzle.compile = function (selector, match / * Internal Use Only * /) {
	...
	if (! caché) {
		...
		// Guardar en caché la función compilada
...
// Soporte: IE <9
// Usa getAttributeNode para obtener valores booleanos cuando getAttribute miente
if (! assert (function (el) {
	...
	init = jQuery.fn.init = función (selector, contexto, raíz) {
		...
		if (typeof selector === "cadena") {
			...
			// (que es equivalente a: $ (contexto) .find (expr)
				...
				return this.constructor (contexto) .find (selector);
			}
...
jQuery.fn.extend ({
	...
	más cercano: función (selectores, contexto) {
			...
			para (; i <l; i ++) {
				para (cur = this [i]; cur && cur! == context; cur = cur.parentNode) {
					...
					if (cur.nodeType <11 && (objetivos?
						...
						// No le pases no elementos a Sizzle
						cur.nodeType === 1 &&
		...
		devuelve this.pushStack (coincidente.length> 1? jQuery.uniqueSort (coincidente): coincidente);
	},
...
function adoptValue (valor, resolver, rechazar, noValue) {
	...
	tratar {
		...
		// Verifique primero el aspecto de la promesa para privilegiar el comportamiento síncrono
							...
							var that = this,
								...
								mightThrow = function () {
									...
									luego = devuelto &&
										...
										// Verifique solo los objetos y funciones para la habilitación
									...
									// Maneja todos los demás valores devueltos
										...
										// y varios valores (comportamiento no especificado)
										...
										(especial || deferred.resolveWith) (eso, argumentos);
									}
								},
...
jQuery.extend ({
	...
	// ¿Está el DOM listo para ser utilizado? Establézcalo en verdadero una vez que ocurra.
	isReady: falso,
...
//
// 1. Aplicar la superficie de la API y la compatibilidad semántica con la rama 1.9.x
...
// 5. Evite exponer los detalles de la implementación en los objetos del usuario (por ejemplo, expandir propiedades)
...
jQuery.extend ({
	...
	_queueHooks: function (elem, type) {
		...
		return dataPriv.get (elem, key) || dataPriv.access (elem, clave, {
...
// Soporte: IE <= 9 solamente
// Acceder a document.activeElement puede lanzar inesperadamente
...
función en (elem, tipos, selector, datos, fn, uno) {
	...
	si (uno === 1) {
		...
		// Use la misma guía para que la persona que llama pueda eliminar usando origFn
...
jQuery.event = {
	...
	agregar: función (elem, tipos, manejador, datos, selector) {
		...
		while (t--) {
			...
			// Debe * haber un tipo, no adjuntar controladores de solo espacio de nombres
			if (! type) {
				Seguir;
			}

			// Si el evento cambia su tipo, use los controladores de eventos especiales para el tipo cambiado
			especial = jQuery.event.special [tipo] || {};

			// Si el selector está definido, determine el tipo de api de evento especial, de lo contrario, se le dará el tipo
			type = (selector? special.delegateType: special.bindType) || escribe;
	...
	controladores: función (evento, controladores) {
		...
		// Agrega los manejadores restantes (enlazados directamente)
		cur = esto;
...
(función () {
	...
	jQuery.extend (apoyo, {
		...
		// establecido en CSS mientras que las propiedades `offset *` informan los valores correctos.
		...
		confiableTrDimensiones: función () {
...
function getWidthOrHeight (elem, dimension, extra) {
	...
	var styles = getStyles (elem),
		...
		// Para evitar forzar un reflujo, solo busque boxSizing si lo necesitamos (gh-4322).
...
jQuery.extend ({
	...
	css: function (elem, name, extra, styles) {
		...
		// Prueba el nombre con prefijo seguido del nombre sin prefijo
		hooks = jQuery.cssHooks [nombre] || jQuery.cssHooks [origName];
...
function genFx (type, includeWidth) {
	...
	para (; i <4; i + = 2 - includeWidth) {
		que = cssExpand [i];
		attrs ["margin" + which] = attrs ["padding" + which] = tipo;
	}
...
function defaultPrefilter (elem, props, opts) {
	...
	for (prop en props) {
		...
		if (rfxtypes.test (valor)) {
			...
			if (valor === (¿oculto? "ocultar": "mostrar")) {
				...
				if (value === "mostrar" && dataShow && dataShow [prop]! == undefined) {
					oculto = verdadero;

				// Ignora todos los demás datos de mostrar / ocultar sin operación
				} demás {
					Seguir;
				}
			}
	...
	// Restringe los estilos de "desbordamiento" y "visualización" durante las animaciones de cuadro
	if (isBox && elem.nodeType === 1) {
		...
		si (mostrar === "ninguno") {
			if (restoreDisplay) {
				...
				// Obtenga valor (s) no vacío forzando temporalmente la visibilidad
			...
			if (jQuery.css (elem, "float") === "none") {
				...
				if (! propTween) {
					...
					if (restoreDisplay == null) {
						...
						restoreDisplay = display === "none"? "" : mostrar;
					}
				}
	...
	animate: function (prop, speed, easing, callback) {
		var vacío = jQuery.isEmptyObject (prop),
			...
			doAnimation = function () {
				...
				// Opere en una copia de la propiedad para que no se pierda la flexibilización por propiedad
...
jQuery.fx.tick = function () {
	...
	para (; i <timers.length; i ++) {
		...
		// Ejecute el temporizador y retírelo de forma segura cuando haya terminado (permitiendo la eliminación externa)
	...
	toggleClass: function (value, stateVal) {
		...
		devuelve this.each (function () {
			...
			// Cambiar el nombre completo de la clase
				...
				// De lo contrario, trae de vuelta lo que fue guardado previamente (si es que hay algo)
...
jQuery.extend (jQuery.event, {
	...
	disparador: función (evento, datos, elem, onlyHandlers) {
		...
		// Clone cualquier dato entrante y anteponga el evento, creando la lista arg del manejador
		datos = datos == nulo?
			[evento]:
...
// los eventos focus (in | out) se activan después de los eventos focus & blur,
	...
	jQuery.each ({focus: "focusin", blur: "focusout"}, function (orig, fix) {
		...
		jQuery.event.special [corrección] = {
			configuración: función () {
				...
				var doc = this.ownerDocument || este.documento || esta,
...
var
	 ...
	 * 5) la ejecución comenzará con el tipo de datos de transporte y ENTONCES continuará hasta "*" si es necesario
	 * /
	prefiltros = {},
	...
	// Evite la secuencia de caracteres comentario-prólogo (# 10098); debe apaciguar la pelusa y evadir la compresión
	allTypes = "* /". concat ("*"),
...
function inspectPrefiltersOrTransports (estructura, opciones, originalOptions, jqXHR) {
	...
	function inspeccionar (dataType) {
		var seleccionado;
		inspeccionado [tipo de datos] = verdadero;
		jQuery.each (estructura [tipo de datos] || [], función (_, prefilterOrFactory) {
	...
	return inspeccionar (options.dataTypes [0]) || ! inspected ["*"] && inspect ("*");
}
...
function ajaxHandleResponses (s, jqXHR, respuestas) {
	...
	// Verifica si estamos tratando con un tipo de contenido conocido
	si (ct) {
	...
	if (dataTypes [0] en las respuestas) {
		...
		// Prueba los tipos de datos convertibles
...
jQuery.extend ({
	...
	ajax: function (url, opciones) {
		...
		// Agregar protocolo si no se proporciona (es posible que los prefiltros lo esperen)
		...
		si (antes de enviar &&
			...
			// Abortar si no lo ha hecho ya y regresar
			return jqXHR.abort ();
		}
		...
		función realizada (estado, nativeStatusText, respuestas, encabezados) {
			...
			// Ignorar las invocaciones repetidas
			if (completado) {
				regreso;
			}
			...
			// Caché de encabezados de respuesta
